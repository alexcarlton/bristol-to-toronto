...
function SidebarButton({ active, calendarColor, calendarId, children }) {
  const { dispatch } = useContext(StateContext);

  const onClick = () => {
    if (active) {
      return dispatch(removeActiveCalendarId(calendarId));
    }

    return dispatch(addActiveCalendarId(calendarId));
  };
