...
describe("calendar page", () => {
  ...
  it("should allow you to deselect a calendar, and remove the events", async () => {
  ...
  it("should allow you to select another calendar, and see the events for that calendar", async () => {
    const { getByText, getByRole, queryByText } = render(<CalendarRoute />);

    await waitFor(() => getByText("Event 1"));

    expect(queryByText("Event 3")).toBe(null);
    expect(queryByText("2 - 4pm")).toBe(null);

    expect(queryByText("Event 4")).toBe(null);
    expect(queryByText("4 - 7pm")).toBe(null);

    const calendarSidebarButton = getByRole("button", {
      name: "Calendar 2",
    });

    act(() => userEvent.click(calendarSidebarButton));
  ...
  it("should show the current day as highlighted", async () => {
    const { getByText } = render(<CalendarRoute />);

    await waitFor(() => getByText("Event 1"));

    expect(document.querySelector("strong.current-day-date").textContent).toBe(
      "24"
    );
  });
});
